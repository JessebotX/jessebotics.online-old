...
export default function ProjectIndex({ projects }: any) {
  ...
  return (
    <RootLayout title="Projects | Jessebot">
      <Navbar />
      <main className="max-w-6xl mx-auto">
        ...
        <ul className="flex flex-wrap mx-auto justify-center m-0 p-0">
          {
            projects.filter((project: Project) => project.name.toLowerCase().includes(input.toLowerCase())).map(({ name, description, link, tools}: Project) => (
              <li key={name} className="p-0 shadow-xl w-[22rem] h-[22rem] m-2 border border-black dark:border-slate-500 hover:scale-105 active:scale-95 transition-all rounded-xl list-none">
                <Link href={link} className="flex flex-col justify-between items-between h-full p-2 text-black dark:text-white no-underline">
                  ...
                  <ul className="flex flex-wrap justify-center p-0 m-0">
                    {
                      tools.map(tool => (
                        <li key={`${name}--${tool}`} className="list-none py-1 px-2.5 bg-purple-500 text-white m-0.5 rounded-full">
                          {tool}
                        </li>
                      ))
                    }
